...
export default function PomodoroStats({
  ...
  return (
    <div className="w-full h-full flex flex-col justify-center items-center p-6 bg-mocha-base/50 backdrop-blur-sm rounded-3xl border border-mocha-surface0">
      <h3 className="text-xl font-bold text-mocha-text mb-4 font-space">
        Daily Progress
      </h3>

      <div className="grid grid-cols-2 gap-8 w-full">
        <div className="flex flex-col items-center justify-center p-4 bg-mocha-surface0 rounded-2xl">
          <span className="text-3xl font-bold text-mocha-mauve">
            {sessionsCompleted}
          </span>
                ...
                if (active && payload && payload.length) {
                  ...
                  return (
                    <div className="bg-mocha-base border border-mocha-surface1 rounded-lg p-3 shadow-lg">
                      <p className="text-mocha-text font-medium text-sm">
                        {data.name}:{" "}
                        <span className="font-bold">{data.value}</span> minutes
                      </p>
                    </div>
                  );
                }
        ...
        {Array.from({ length: 4 }).map((_, i) => (
          <div
            ...
            className={`w-3 h-3 rounded-full transition-all duration-300 ${
              i < sessionsCompleted
                ? "bg-mocha-mauve scale-110"
                : "bg-mocha-surface1"
            }`}
          />
        ))}
        {sessionsCompleted > 4 && (
          <span className="text-xs text-mocha-subtext0 ml-1">
            +{sessionsCompleted - 4}
          </span>
        )}
      </div>
