...
const features = [
  {
    ...
    description:
      "Hands-free interaction initiated by a custom wake word, allowing you to stay focused on your work.",
  },
  {
    icon: <Bot className="h-8 w-8 text-mocha-mauve" />,
    title: "Switchable AI Tutors",
    description:
      'Select different modes like "Socratic Tutor" for guiding questions or "Direct Answer" for quick help.',
  },
  {
    icon: <Smile className="h-8 w-8 text-mocha-yellow" />,
    title: "Dynamic Facial Expressions",
    description:
      "A color TFT screen displays rich, animated faces to convey listening, thinking, and speaking states.",
  },
  {
    icon: <Timer className="h-8 w-8 text-mocha-peach" />,
    title: "Integrated Pomodoro Timer",
    description:
      "Actively coaches students on time management and focus skills to maximize productivity.",
  },
  {
    icon: <Camera className="h-8 w-8 text-mocha-blue" />,
    title: "Visual Problem Solving",
    description:
      "Utilizes an onboard camera to interpret problems from physical documents and provide AI-powered help.",
  },
  {
    icon: <Smartphone className="h-8 w-8 text-mocha-lavender" />,
    title: "Companion Web App Integration",
    description:
      "Communicates with a backend to sync history and settings with a companion web portal.",
  },
];
        ...
        <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 justify-center">
          {features.map((feature, index) => (
            ...
            >
              ...
              <p className="text-mocha-subtext0">{feature.description}</p>
            </div>
          ))}
        </div>
      </div>
